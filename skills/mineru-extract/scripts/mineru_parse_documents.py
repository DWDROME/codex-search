...
Notes
- This is NOT an MCP server. It's a script meant to be called by Codex skills via exec.
...
def _default_workspace() -> pathlib.Path:
    ...
    if v := os.environ.get("MINERU_WORKSPACE"):
        return pathlib.Path(v)
    if v := os.environ.get("CODEX_WORKSPACE"):
        return pathlib.Path(v)
    project_root = pathlib.Path(__file__).resolve().parents[3]
    return project_root / ".runtime" / "codex-workspace"
...
def main() -> int:
    ...
    ap.add_argument("--model-version", default=None, choices=["pipeline", "vlm", "MinerU-HTML"], help="pipeline | vlm | MinerU-HTML")
    ...
    if not sources:
        print(
            json.dumps(
                {
                    "ok": False,
                    "error": "No valid source found in --file-sources.",
                    "items": [],
                    "errors": [],
                },
                ensure_ascii=False,
        ...
        return 2
    if args.model_version == "MinerU-HTML" and args.page_ranges:
        print(
            json.dumps(
                {
                    "ok": False,
                    "error": "--page-ranges is not supported with MinerU-HTML model.",
                    "items": [],
                    "errors": [],
                },
                ensure_ascii=False,
