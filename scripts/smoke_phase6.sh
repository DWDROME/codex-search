...
run_step "compileall" run_uv python -m compileall "$ROOT_DIR/src"
run_step "cli_help" run_uv python -m codex_search_stack.cli --help

run_json_step \
  "search_deep" \
  'assert isinstance(payload.get("results"), list), "missing results"' \
  run_uv python -m codex_search_stack.cli search "codex 多源搜索 号池 回退" --mode deep --intent exploratory --num 3

run_json_step \
  "extract_basic" \
  'assert payload.get("ok") is True, "extract not ok"; assert payload.get("engine"), "missing engine"' \
  run_uv python -m codex_search_stack.cli extract "https://en.wikipedia.org/wiki/Web_scraping" --max-chars 2000
...
run_json_step \
  ...
  run_uv python -m codex_search_stack.cli explore "microsoft/graphrag" --issues 3 --commits 3 --external-num 4 --extract-top 1 --format json
...
run_json_step \
  ...
  run_uv python -m codex_search_stack.cli explore "microsoft/graphrag" --issues 3 --commits 3 --external-num 4 --extract-top 1 --confidence-profile quick --format json

run_text_step \
  "explore_markdown" \
  "profile=quick" \
  run_uv python -m codex_search_stack.cli explore "microsoft/graphrag" --issues 3 --commits 3 --external-num 4 --extract-top 1 --confidence-profile quick --format markdown
